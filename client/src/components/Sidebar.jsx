...
const deleteChat = async (e,chatId)=>{
  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b from-[242124]/30 to-[#000000]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
        {/* logo */}
        ...
        <div onClick={()=>{navigate('/credits'); setIsMenuOpen(false)}} className='flex items-center gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
            ...
            <div className='flex flex-col text-sm'>
                   ...
                   <p className='tex-xs text-gray-400'>Purchase credits to use quickgpt</p>
            </div>
        </div>
        ...
        {/* user account //created by Nishikant */}
